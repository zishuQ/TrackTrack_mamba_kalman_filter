...
# 配置
MOT17_ROOT = "/home/shang/datasets/MOT17/train"
OUTPUT_JSON = "/home/shang/workspace/TrackTrack/1. YOLOX/jsons/mot17_train.json"

# Val集使用的序列（需要排除）
VAL_SEQS = ['MOT17-04-FRCNN', 'MOT17-05-FRCNN', 'MOT17-09-FRCNN']

# Train集应该包含的序列
TRAIN_SEQS = ['MOT17-02-FRCNN', 'MOT17-10-FRCNN', 'MOT17-11-FRCNN', 'MOT17-13-FRCNN']
...
def create_coco_json():
    ...
    print(f"处理训练序列: {TRAIN_SEQS}")
    
    for seq_name in sorted(TRAIN_SEQS):
        seq_path = os.path.join(MOT17_ROOT, seq_name)
        ...
        for frame_id in range(1, seq_length + 1):
            # 添加图像信息
            img_filename = f"MOT17/train/{seq_name}/img1/{frame_id:06d}{img_ext}"
...
if __name__ == '__main__':
    ...
    if not os.path.exists(MOT17_ROOT):
        print(f"错误: MOT17数据集不存在: {MOT17_ROOT}")
        print("请修改脚本中的 MOT17_ROOT 路径")
        exit(1)

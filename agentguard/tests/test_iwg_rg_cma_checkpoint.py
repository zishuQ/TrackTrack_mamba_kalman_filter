...
from agentguard.datasets.iwg_rg_cma_dataset import PACKED_TRAIN_DATA_FORMAT
...
def test_iwg_rg_cma_checkpoint_strict_roundtrip(tmp_path):
    ...
    validate_iwg_rg_cma_checkpoint_contract(legacy)
...
def test_memory_shards_respect_truncated_packed_dataset():
    class DatasetStub:
        metadata = {
            "format": PACKED_TRAIN_DATA_FORMAT,
            "dataset": "MOT17",
            "train_sequences": ["MOT17-A", "MOT17-B"],
            "timeline_counts": {
                "MOT17-A": {"labeled_endpoints": 11},
                "MOT17-B": {"labeled_endpoints": 7},
            },
        }
    ...
    schedule = build_memory_shard_schedule(
        DatasetStub(),
        {"epochs": 2, "memory_shards": 2, "epochs_per_shard": 1, "shard_cycles": 1},
    )
    assert [sum(item["samples"] for item in phase["ranges"]) for phase in schedule["phases"]] == [2, 3]
    assert sum(phase["samples"] for phase in schedule["phases"]) == 5
...
def test_low_memory_training_switches_phases_without_resetting_progress(tmp_path):
    ...
    assert not (tmp_path / "iwg_rg_cma_epoch001.pt").exists()

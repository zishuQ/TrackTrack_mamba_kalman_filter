...
# Controlled MOT17 reliability-input ablation.
# Only scalar63 evidence is masked; labels, model dimensions, bound, context,
# loss, sample exposure, and optimizer schedule stay at the old baseline.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PY="${PYTHON_BIN:-${ROOT}/.venv/bin/python}"
RUN_NAME="${RUN_NAME:-iwg_rg_cma_mot20e100_to_mot17_reliability_noscalar_finetune50_bound005_seed42_bs1024}"
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/agentguard/run_iwg_rg_cma_reliability_noscalar_mot17_finetune50.sh
...
Only the reliability scalar63 input is masked. The old loss settings remain:
residual_beta=1.0, residual_weight=0.5, revision_weight=0.01,
...
MODEL=correction_bound:0.05,context_size:6,reliability_mode:no-scalar
...
echo "$(date '+%Y-%m-%d %H:%M:%S') | TRAIN MOT17 reliability no-scalar" \
...
"${PY}" "${ROOT}/scripts/agentguard/package_mot17_submission.py" \
  ...
  --output-zip "${RUN_ROOT}/MOT17_test_reliability_noscalar_final_post.zip" \
  --manifest "${RUN_ROOT}/submission_manifest.json" \
  2>&1 | tee "${RUN_ROOT}/logs/submission.log"

touch "${RUN_ROOT}/training_completed"
complete=1
echo "$(date '+%Y-%m-%d %H:%M:%S') | DONE MOT17 reliability no-scalar" \

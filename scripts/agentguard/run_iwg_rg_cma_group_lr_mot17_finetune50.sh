...
# MOT17 warm-start experiment with separate Base IWG and RG-CMA learning rates.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PY="${PYTHON_BIN:-${ROOT}/.venv/bin/python}"
RUN_NAME="${RUN_NAME:-iwg_rg_cma_mot20e100_to_mot17_group_lr05_finetune50_bound005_seed42_bs1024}"
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/agentguard/run_iwg_rg_cma_group_lr_mot17_finetune50.sh

Stages:
  1. Warm-start MOT17 for 50 epochs from MOT20 epoch100.
  2. Validate the checkpoint on the MOT17 training dataset.
  3. Evaluate MOT17 train with base/final raw gates.
  4. Run MOT17 test with final gate + postprocessing and package it.

The only changed training variable is the optimizer parameter-group learning
rate: Base IWG=1e-5 and RG-CMA=5e-6. The model, labels, data exposure,
loss, bound, context, batch, and scheduler settings remain unchanged.
...
Environment overrides:
  ...
  --dry-run   Print paths and configuration without running.
EOF
}
...
MODEL=correction_bound:0.05,context_size:6,reliability_mode:full
LR=shared:1e-5,base_iwg:1e-5,rg_cma:5e-6
LOSS=residual_beta:1.0,residual_weight:0.5,revision_weight:0.01,hard_example_gain:0.0
EVALUATION=train:base+final,raw;test:final,post
EOF
  exit 0
fi
...
if [[ -e "${RUN_ROOT}/completed" || -e "${RUN_ROOT}/running" ]]; then
  echo "Experiment already has an active/completed marker: ${RUN_ROOT}" >&2
  exit 2
fi
...
echo "$(date '+%Y-%m-%d %H:%M:%S') | TRAIN MOT17 group learning rate" \
...
run_raw_case base "${RUN_NAME}_base_raw" mot17_base_raw
...
require_file "${TRACKER_OUTPUT_ROOT}/${TEST_FOLDER}/MOT17-01-FRCNN.txt"

"${PY}" "${ROOT}/scripts/agentguard/package_mot17_submission.py" \
  --source-dir "${TRACKER_OUTPUT_ROOT}/${TEST_FOLDER}" \
  --output-zip "${RUN_ROOT}/MOT17_test_group_lr05_final_post.zip" \
  --manifest "${RUN_ROOT}/submission_manifest.json" \
  2>&1 | tee "${RUN_ROOT}/logs/submission.log"

touch "${RUN_ROOT}/training_completed"
complete=1
echo "$(date '+%Y-%m-%d %H:%M:%S') | DONE MOT17 group learning rate" \

...
# Train the 1x6 residual-target experiment from the existing public compact
# dataset. This script deliberately does not build caches, labels, or data.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PY="${PYTHON_BIN:-${ROOT}/.venv/bin/python}"
RUN_NAME="${RUN_NAME:-iwg_rg_cma_1x6_residual_target_sportsmot_trainval_bound005_sampleprop_seed42_bs1024_shard4x1_200e}"
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/agentguard/run_iwg_rg_cma_1x6_residual_v2_sportsmot_trainval.sh

This runs:
  1. 1x6 SportsMOT trainval training for 200 nominal epochs.
  2. Checkpoint validation.
  3. SportsMOT val raw for base and final.

Data policy:
  Reuses outputs/agentguard/datasets/iwg_rg_cma/SportsMOT/nsa_trainval_v3_compact.
  Does not build labels, event caches, detection caches, or a new dataset.
  Does not run test, post-processing, or AFLink.
...
LOSS=residual_target_mode:oracle-confidence,residual_beta:1.0,residual_weight:0.5,revision_weight:0.01,no_harm_weight:0.0
...
# Verify that the old compact files contain the encoded oracle/confidence
# fields. No files are written by this check.
"${PY}" - "${DATASET_DIR}" <<'PY'
import json
import sys
...
available = set(metadata.get("compact_index_array_files", {}))
if not {"risk_target", "cue_target"}.issubset(available):
    raise SystemExit(
        "existing dataset cannot derive oracle-confidence targets: "
        "risk_target/cue_target are missing"
    )
if int(metadata.get("num_train_samples", 0)) <= 0:
    raise SystemExit("public SportsMOT dataset has no training samples")
...
try:
    ...
    for key in ("oracle_gate_target", "gate_confidence"):
        if key not in sample:
            raise SystemExit(f"dataset sample did not derive {key}")
    if tuple(sample["oracle_gate_target"].shape) != (2,):
        raise SystemExit("derived oracle_gate_target has the wrong shape")
    if tuple(sample["gate_confidence"].shape) != (2,):
        raise SystemExit("derived gate_confidence has the wrong shape")
    print(
        "dataset=SportsMOT",
        "split=trainval",
        "samples=", metadata["num_train_samples"],
        "context_size=", metadata["context_size"],
        "target_source=derived_from_risk_target_and_cue_target",
        "oracle=", sample["oracle_gate_target"].tolist(),
        "confidence=", sample["gate_confidence"].tolist(),
    )
finally:
    dataset.close()
PY
...
TRAIN_COMMAND=(
  ...
  --no-harm-weight 0.0
  --residual-target-mode oracle-confidence
...
echo "$(date '+%Y-%m-%d %H:%M:%S') | TRAIN SportsMOT 1x6 residual target" \
...
run_val_case() {
  ...
  local suffix="$2"
  local log_name="$3"
  local folder="sportsmot_val_0.80_${suffix}_iwg_rg_cma_${output_mode}"
  ...
  if [[ -e "${TRACKER_OUTPUT_ROOT}/${folder}" ]]; then
    echo "Refusing to overwrite existing validation output: ${TRACKER_OUTPUT_ROOT}/${folder}" >&2
    exit 2
  fi

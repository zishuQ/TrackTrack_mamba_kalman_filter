...
# 1x6 clean cross-modal experiment under the old best training contract:
#   1. MOT17 warm-start from the compatible 1x6 MOT20 epoch100 checkpoint
#   2. SportsMOT trainval from scratch, using the old 4-shard schedule
#
# The 1x6 architecture is legacy-clean-cross-modal: each modality uses an
# endpoint query over six history tokens, followed by two-token fusion. Labels
# and public datasets are reused; this script never rebuilds labels.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PY="${PYTHON_BIN:-${ROOT}/.venv/bin/python}"
PIPELINE_NAME="${PIPELINE_NAME:-iwg_rg_cma_1x6_oldbest_mot17_sportsmot_serial_20260724}"
...
MOT20_INIT_CHECKPOINT="${EXPERIMENT_ROOT}/iwg_rg_cma_mot20_legacy_clean_cross_modal_seed42_100e/checkpoints/iwg_rg_cma_epoch100.pt"

MOT17_RUN="${EXPERIMENT_ROOT}/iwg_rg_cma_1x6_oldbest_mot20e100_to_mot17_bound005_sampleprop_seed42_bs1024_finetune50"
SPORTS_RUN="${EXPERIMENT_ROOT}/iwg_rg_cma_1x6_oldbest_sportsmot_trainval_bound005_sampleprop_seed42_bs1024_shard4x1_200e"
...
while (($#)); do
  case "$1" in
    -h|--help)
      cat <<'EOF'
Usage:
  bash scripts/agentguard/run_iwg_rg_cma_1x6_oldbest_mot17_sportsmot_serial.sh

Stages:
  1. MOT17 all/FRCNN, warm-start from the compatible 1x6 MOT20 epoch100,
     50 epochs, lr=1e-5, one full-data phase.
  2. SportsMOT trainval, fresh 1x6 model, 200 epochs,
     4 shards x 1 epoch x 50 cycles.

Both stages use bound=0.05, context=6, sample-proportional sampling,
batch=1024, seed=42, weight decay=1e-4, warmup=1, grad clip=1.0,
and the default residual/revision losses. After each stage the checkpoint is
validated and final raw tracker metrics are evaluated; no post or test run is
...
MODEL=temporal_attention:1x6,two_token_fusion:true,bound:0.05,context:6,reliability:full
...
EVALUATION=MOT17:all/final/raw;SportsMOT:val/final/raw;post:none;test:none
EOF
  exit 0
fi
...
def check_dataset(path, dataset, split, index_format, samples):
    ...
    actual = {
        ...
        "index_format": metadata.get("index_format", "jsonl_v1"),
...
variant = "legacy-clean-cross-modal"
contract = model_contract(correction_bound=0.05, context_size=6, architecture_variant=variant)
if contract["model_schema"] != "agentguard_iwg_rg_cma_legacy_clean_cross_modal_v1":
    raise SystemExit(f"unexpected 1x6 model schema: {contract['model_schema']!r}")
...
printf '%s\n' \
  "architecture_variant=${ARCHITECTURE_VARIANT}" \
  "temporal_attention=1x6" \
  "two_token_fusion=true" \
  "correction_bound=0.05" \
  "context_size=6" \
  "sequence_sampling=sample-proportional" \
  "mot17_lr=1e-5" \
...
stage "START 1x6 old-best MOT17 migration + SportsMOT serial pipeline"

# MOT17 intentionally omits shard arguments. The trainer defaults to one
# full-data phase; the warm-start contract resolves it to 1 x 1 x 50.
train_stage \
  "MOT17 from compatible 1x6 MOT20 epoch100" \
  "${MOT17_RUN}" \
  "${MOT17_DATASET}" \
  50 0.00001 "${MOT17_CHECKPOINT}" \
  "${MOT20_INIT_CHECKPOINT}"
eval_final_raw \
  "MOT17 1x6 migration epoch050" \
  "${MOT17_RUN}" MOT17 all "${MOT17_CHECKPOINT}" \
  mot17_1x6_final_all_raw \
  iwg_rg_cma_1x6_oldbest_mot17_e050_final_all \
  "${MOT17_SEQUENCES[@]}"

train_stage \
  "SportsMOT 1x6 from scratch" \
  "${SPORTS_RUN}" \
  "${SPORTS_DATASET}" \
  200 0.0001 "${SPORTS_CHECKPOINT}" \
  "" 4 1 50
eval_final_raw \
  "SportsMOT 1x6 epoch200" \
  "${SPORTS_RUN}" SportsMOT val "${SPORTS_CHECKPOINT}" \
  sportsmot_1x6_final_val_raw \
  iwg_rg_cma_1x6_oldbest_sportsmot_e200_final_val \
  "${SPORTS_SEQUENCES[@]}"

stage "DONE 1x6 old-best MOT17 migration + SportsMOT serial pipeline"
pipeline_complete=1

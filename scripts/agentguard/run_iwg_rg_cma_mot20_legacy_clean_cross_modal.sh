...
SCRIPT_PATH="${ROOT}/scripts/agentguard/run_iwg_rg_cma_mot20_legacy_clean_cross_modal.sh"
PY="${PYTHON_BIN:-${ROOT}/.venv/bin/python}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-iwg_rg_cma_mot20_legacy_clean_cross_modal_seed42_100e}"
...
LEGACY_BEST_HOTA="0.792449"

usage() {
  cat <<'EOF'
Usage:
  bash scripts/agentguard/run_iwg_rg_cma_mot20_legacy_clean_cross_modal.sh

Options:
  --dry-run    Print the fixed experiment contract and output paths.
  --preflight  Validate data, model contract, CLI arguments, and CUDA only.

Experiment:
  MOT20/all, legacy Base-IWG + clean motion/appearance RG-CMA,
  context=6, bound=0.05, seed=42, batch=1024, lr=1e-4,
  4 shards x 1 epoch x 25 cycles = 100 nominal epochs = 25 full passes.

Evaluation:
  checkpoint validation -> base raw -> final raw. No post and no test split.
EOF
}
...
LEGACY_BEST_HOTA=${LEGACY_BEST_HOTA}
EOF
  exit 0
fi
...
git -C "${ROOT}" status --porcelain > "${EXPERIMENT_ROOT}/provenance/git_status_porcelain.txt"
sha256sum \
...
echo "$(date '+%Y-%m-%d %H:%M:%S') | TRAIN ${ARCHITECTURE_VARIANT}" \
  | tee -a "${EXPERIMENT_ROOT}/stages.log"
"${train_command[@]}" 2>&1 | tee "${EXPERIMENT_ROOT}/logs/train.log"
require_file "${CHECKPOINT}"

echo "$(date '+%Y-%m-%d %H:%M:%S') | VALIDATE CHECKPOINT" \
  | tee -a "${EXPERIMENT_ROOT}/stages.log"
...
run_raw_case() {
  ...
  local folder="mot20_all_0.80_${suffix}_iwg_rg_cma_${output}"
  ...
  printf '%q ' "${command[@]}" > "${EXPERIMENT_ROOT}/provenance/mot20_${output}_raw.command.txt"
...
echo "$(date '+%Y-%m-%d %H:%M:%S') | EVAL BASE RAW" \
  | tee -a "${EXPERIMENT_ROOT}/stages.log"
run_raw_case base
base_folder="${RAW_FOLDER}"

echo "$(date '+%Y-%m-%d %H:%M:%S') | EVAL FINAL RAW" \
  | tee -a "${EXPERIMENT_ROOT}/stages.log"
run_raw_case final
...
"${PY}" - \
  ...
  "${base_folder}" "${final_folder}" "${LEGACY_BEST_HOTA}" \
...
legacy_best_hota = float(sys.argv[6])
...
manifest = {
    ...
    "reference": {
        ...
        "source": "agentguard/IWG_RG_CMA_HYPERPARAMETER_PLAN.md",
    },
    "comparisons": {
